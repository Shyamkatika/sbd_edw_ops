...
{% macro Full_Refresh_Steps(Dest_Table,Source_view) %}
    ...
    {% if resrecheck_main_table!=0 %}
        {{ log("resrecheck_main_table-2:" ~resrecheck_main_table)}}
          BEGIN;
              ...
              {% set Create_tmp2 %}
                    CREATE OR REPLACE TABLE {{var('db_srcviewdb')}}.{{tgttable_tmp2}} copy grants
                    AS SELECT *  FROM  {{var('db_factsschema')}}.{{tgttable}} WHERE {{var('column_verexpirydt')}} <> '9999-12-31 00:00:00.000';
              {% endset %}

              {% if execute %}
                    {%- set result = run_query(Create_tmp2) %}
              {% else %} 
                    {{return(false)}}
              {% endif %}       
