...
{% macro Scd2_Steps_BKP(Dest_Table,Source_view,vzone4dte,vload_type,tgtpk,vsid,srctblname,srcschema) %}
    ...
    {%- set rescheck_main_table    = resultqry.columns[0].values()[0] -%}
      ...
      BEGIN;
              ...
              {% set Create_tmp_incrdte %}
                    ...
                    as SELECT * FROM {{var('db_srcviewdb')}}.{{Source_view}} where ZONE4_ETL_UPD_DTE > '{{vzone4dte}}' OR EXPR_DTE > '{{vzone4dte}}';
              {% endset %}
              ...
              {% set insert_stmnt %}
                    INSERT INTO {{var('db_factsschema')}}.{{tgttable}} 
                    SELECT * FROM  {{var('db_srcviewdb')}}.{{tgttable_tmp}};
              {% endset %}
