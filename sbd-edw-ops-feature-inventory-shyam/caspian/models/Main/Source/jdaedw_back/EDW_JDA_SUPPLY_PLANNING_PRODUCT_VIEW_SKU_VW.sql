Select
CONCAT(coalesce(SKU.ITEM::varchar,''),'~',coalesce(SKU.LOC::varchar,'')) AS SUPPLY_PLNG_PROD_VW_KEY,
'JDAEDW' AS {{var('column_srcsyskey')}},
MD5(CONCAT(coalesce(SKU.ITEM::varchar,''),'~',coalesce(SKU.LOC::varchar,''))) AS {{var('column_rechashkey')}},
...
case when length(trim(UPPER((SKU.ITEM)))) <1 then {{var('default_mapkey')}} when UPPER(SKU.ITEM)
IS NULL then {{var('default_mapkey')}} else UPPER(SKU.ITEM) end AS PROD_KEY,
...
case when length(trim(UPPER((SKU.LOC )))) <1 then {{var('default_mapkey')}} when UPPER(SKU.LOC )
IS NULL then {{var('default_mapkey')}} else UPPER(SKU.LOC ) end AS LOC_KEY,
...
--CASE WHEN SKUDISPLAY.SEQ_INT_ENABLE_SW = '1' THEN 'Y' ELSE 'N' END AS SEQ_ENABLE_FLAG, --NA in SF
...
FROM {{source('JDAEDW','SKU')}} SKU  
LEFT JOIN {{source('JDAEDW','SKUDISPLAY')}} SKUDISPLAY ON SKU.ITEM::varchar  = SKUDISPLAY.ITEM::varchar  and SKU.LOC::varchar  = SKUDISPLAY.LOC::varchar  
--where CONCAT(coalesce(SKU.ITEM::varchar,''),'~',coalesce(SKU.LOC::varchar,'')) NOT IN (select CONCAT(coalesce(SKUDISPLAY.ITEM::varchar,''),'~',coalesce(SKUDISPLAY.LOC::varchar,''))
--FROM  {{source('JDA','SKUDISPLAY')}} SKUDISPLAY LEFT JOIN  {{source('JDA','SKU')}} SKU ON SKUDISPLAY.ITEM::varchar  = SKU.ITEM::varchar  and SKUDISPLAY.LOC::varchar  = SKU.LOC::varchar  )
