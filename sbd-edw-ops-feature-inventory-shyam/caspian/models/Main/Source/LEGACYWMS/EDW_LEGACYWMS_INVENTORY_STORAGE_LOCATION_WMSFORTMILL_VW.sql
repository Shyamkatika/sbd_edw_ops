select
UPPER(CONCAT(coalesce(LOCATOR.LODNUM ::VARCHAR,''),'~','WMSFORTMILL')) AS INVTY_SLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATOR.LODNUM::VARCHAR,''),'~','WMSFORTMILL'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSFORTMILL','LOCATOR')}} LOCATOR
