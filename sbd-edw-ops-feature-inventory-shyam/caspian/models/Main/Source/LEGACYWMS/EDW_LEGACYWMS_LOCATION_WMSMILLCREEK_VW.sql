select
UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSMILLCREEK')) AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSMILLCREEK'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSMILLCREEK','LOCATION')}} LOCATION