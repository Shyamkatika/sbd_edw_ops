SELECT 
UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSJACKSON')) AS PROD_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5 (UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSJACKSON'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSJACKSON','ITEM')}} ITEM