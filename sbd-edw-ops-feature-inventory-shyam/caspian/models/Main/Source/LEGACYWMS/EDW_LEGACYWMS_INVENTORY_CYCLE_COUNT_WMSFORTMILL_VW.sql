select
UPPER(CONCAT(coalesce(CYCLE_COUNT.CC_NUMBER::varchar,''),'~',coalesce(CYCLE_COUNT.LOCATION::varchar,''),'~',coalesce(CYCLE_COUNT.SKU::varchar,''),'~','WMSFORTMILL')) AS INVTY_CYCLE_CNT_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(CYCLE_COUNT.CC_NUMBER::varchar,''),'~',coalesce(CYCLE_COUNT.LOCATION::varchar,''),'~',coalesce(CYCLE_COUNT.SKU::varchar,''),'~','WMSFORTMILL'))) AS {{var('column_rechashkey')}},
...
'WMSFORTMILL' as WMS_SCHEMA_NAME,
...
CASE WHEN (CYCLE_COUNT.DISCREPANCY_FLAG) ='Y' THEN 'Y' ELSE 'N' END	AS 	DISCREPANCY_FLAG,
...
FROM {{source('WMSFORTMILL','CYCLE_COUNT')}} CYCLE_COUNT