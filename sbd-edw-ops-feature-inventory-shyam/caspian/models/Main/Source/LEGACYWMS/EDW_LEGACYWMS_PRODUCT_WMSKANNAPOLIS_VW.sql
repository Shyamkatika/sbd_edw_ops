SELECT 
UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSKANNAPOLIS')) AS PROD_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5 (UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSKANNAPOLIS'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSKANNAPOLIS','ITEM')}} ITEM