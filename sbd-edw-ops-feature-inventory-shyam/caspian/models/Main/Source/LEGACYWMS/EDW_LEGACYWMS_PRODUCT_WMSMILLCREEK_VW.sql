SELECT 
UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSMILLCREEK')) AS PROD_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5 (UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSMILLCREEK'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSMILLCREEK','ITEM')}} ITEM