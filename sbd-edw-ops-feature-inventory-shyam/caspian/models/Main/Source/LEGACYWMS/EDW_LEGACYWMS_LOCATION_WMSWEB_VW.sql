select
CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSWEB') AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSWEB')) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSWEB','LOCATION')}} LOCATION