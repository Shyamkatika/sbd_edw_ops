select
UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSJACKSON')) AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSJACKSON'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSJACKSON','LOCATION')}} LOCATION