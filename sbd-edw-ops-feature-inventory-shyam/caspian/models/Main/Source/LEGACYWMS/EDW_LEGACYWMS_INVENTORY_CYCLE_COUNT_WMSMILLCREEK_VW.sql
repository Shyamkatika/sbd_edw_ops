select
UPPER(CONCAT(coalesce(CYCLE_COUNT.CC_NUMBER::varchar,''),'~',coalesce(CYCLE_COUNT.LOCATION::varchar,''),'~',coalesce(CYCLE_COUNT.SKU::varchar,''),'~','WMSMILLCREEK')) AS INVTY_CYCLE_CNT_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(CYCLE_COUNT.CC_NUMBER::varchar,''),'~',coalesce(CYCLE_COUNT.LOCATION::varchar,''),'~',coalesce(CYCLE_COUNT.SKU::varchar,''),'~','WMSMILLCREEK'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSMILLCREEK','CYCLE_COUNT')}} CYCLE_COUNT