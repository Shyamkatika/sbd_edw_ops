select
UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSBREWSTER')) AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSBREWSTER'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSBREWSTER','LOCATION')}} LOCATION