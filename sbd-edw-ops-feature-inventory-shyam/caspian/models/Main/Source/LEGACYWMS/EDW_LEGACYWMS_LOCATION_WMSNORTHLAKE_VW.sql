select
UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSNORTHLAKE')) AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSNORTHLAKE'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSNORTHLAKE','LOCATION')}} LOCATION