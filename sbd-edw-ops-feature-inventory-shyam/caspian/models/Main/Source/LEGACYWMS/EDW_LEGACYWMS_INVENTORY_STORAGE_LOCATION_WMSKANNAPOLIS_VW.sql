select
UPPER(CONCAT(coalesce(LOCATOR.LODNUM ::VARCHAR,''),'~','WMSKANNAPOLIS')) AS INVTY_SLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATOR.LODNUM::VARCHAR,''),'~','WMSKANNAPOLIS'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSKANNAPOLIS','LOCATOR')}} LOCATOR
