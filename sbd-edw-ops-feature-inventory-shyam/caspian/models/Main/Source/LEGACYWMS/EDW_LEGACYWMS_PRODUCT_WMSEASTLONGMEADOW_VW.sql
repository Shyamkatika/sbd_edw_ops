SELECT 
UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSEASTLONGMEADOW')) AS PROD_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5 (UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSEASTLONGMEADOW'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSEASTLONGMEADOW','ITEM')}} ITEM