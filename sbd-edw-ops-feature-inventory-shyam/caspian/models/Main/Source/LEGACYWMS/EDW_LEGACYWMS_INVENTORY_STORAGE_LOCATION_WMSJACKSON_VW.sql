select
UPPER(CONCAT(coalesce(LOCATOR.LODNUM ::VARCHAR,''),'~','WMSJACKSON')) AS INVTY_SLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATOR.LODNUM::VARCHAR,''),'~','WMSJACKSON'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSJACKSON','LOCATOR')}} LOCATOR
