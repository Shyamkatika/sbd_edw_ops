select
UPPER(CONCAT(coalesce(LOCATOR.LODNUM ::VARCHAR,''),'~','WMSNORTHLAKE')) AS INVTY_SLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATOR.LODNUM::VARCHAR,''),'~','WMSNORTHLAKE'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSNORTHLAKE','LOCATOR')}} LOCATOR
