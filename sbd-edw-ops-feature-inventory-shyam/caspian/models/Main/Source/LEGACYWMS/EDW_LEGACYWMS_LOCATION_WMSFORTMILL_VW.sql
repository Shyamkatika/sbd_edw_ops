select
CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSFORTMILL') AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSFORTMILL')) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSFORTMILL','LOCATION')}} LOCATION