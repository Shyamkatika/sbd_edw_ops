select
CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSEASTLONGMEADOW') AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSEASTLONGMEADOW')) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSEASTLONGMEADOW','LOCATION')}} LOCATION