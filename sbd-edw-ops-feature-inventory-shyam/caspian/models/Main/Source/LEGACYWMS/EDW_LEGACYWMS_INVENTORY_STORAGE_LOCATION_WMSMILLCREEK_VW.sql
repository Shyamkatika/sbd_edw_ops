select
UPPER(CONCAT(coalesce(LOCATOR.LODNUM ::VARCHAR,''),'~','WMSMILLCREEK')) AS INVTY_SLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATOR.LODNUM::VARCHAR,''),'~','WMSMILLCREEK'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSMILLCREEK','LOCATOR')}} LOCATOR
