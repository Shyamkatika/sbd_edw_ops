select
UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSFONTANA')) AS LOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATION.LOCATION ::VARCHAR,''),'~','WMSFONTANA'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSFONTANA','LOCATION')}} LOCATION