SELECT 
UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSNORTHLAKE')) AS PROD_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5 (UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSNORTHLAKE'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSNORTHLAKE','ITEM')}} ITEM