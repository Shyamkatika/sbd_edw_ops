Select
UPPER(CONCAT(coalesce(INVENTORY.LODNUM ::VARCHAR,''),'~',coalesce(INVENTORY.SKU :: VARCHAR,''),'~','WMSJACKSON')) AS INVTY_ALLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(INVENTORY.LODNUM::VARCHAR,''),'~',coalesce(INVENTORY.SKU :: VARCHAR,''),'~','WMSJACKSON'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSJACKSON','INVENTORY')}} INVENTORY