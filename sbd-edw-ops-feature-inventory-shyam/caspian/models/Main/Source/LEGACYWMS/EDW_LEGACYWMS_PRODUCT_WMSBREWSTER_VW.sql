SELECT 
UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSBREWSTER')) AS PROD_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5 (UPPER(CONCAT(COALESCE(ITEM.SKU,''),'~','WMSBREWSTER'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSBREWSTER','ITEM')}} ITEM