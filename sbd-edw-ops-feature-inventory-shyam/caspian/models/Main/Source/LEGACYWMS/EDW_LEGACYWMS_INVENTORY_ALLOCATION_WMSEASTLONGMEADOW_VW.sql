select
UPPER(CONCAT(coalesce(INVENTORY.LODNUM ::VARCHAR,''),'~',coalesce(INVENTORY.SKU :: VARCHAR,''),'~','WMSEASTLONGMEADOW')) AS INVTY_ALLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(INVENTORY.LODNUM::VARCHAR,''),'~',coalesce(INVENTORY.SKU :: VARCHAR,''),'~','WMSEASTLONGMEADOW'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSEASTLONGMEADOW','INVENTORY')}} INVENTORY