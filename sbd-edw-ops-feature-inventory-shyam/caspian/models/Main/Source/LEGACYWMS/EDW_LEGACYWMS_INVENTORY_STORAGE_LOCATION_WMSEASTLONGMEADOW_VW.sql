select
UPPER(CONCAT(coalesce(LOCATOR.LODNUM ::VARCHAR,''),'~','WMSEASTLONGMEADOW')) AS INVTY_SLOC_KEY,
'LEGACYWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(LOCATOR.LODNUM::VARCHAR,''),'WMSEASTLONGMEADOW'))) AS {{var('column_rechashkey')}},
...
FROM {{source('WMSEASTLONGMEADOW','LOCATOR')}} LOCATOR
