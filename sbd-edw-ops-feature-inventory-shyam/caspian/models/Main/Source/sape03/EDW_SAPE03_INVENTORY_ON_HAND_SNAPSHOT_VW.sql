...
---case  when UPPER(MARD.PSTAT) ='X' then 'Y' WHEN UPPER(MARD.PSTAT) IS NULL then 'U'  else 'N'  end	as	MTNC_STAT_TYP_LKEY	,

case when length(trim(UPPER((MARD.PSTAT)))) <1 then {{var('default_mapkey')}} when UPPER(MARD.PSTAT)
IS NULL then {{var('default_mapkey')}} else UPPER(MARD.PSTAT) end as MTNC_STAT_TYP_LKEY ,
...
NULL	as	PAST_RESTRCT_BATCHES_TOT_QTY	,
NULL	as	PAST_INSPCT_STK_QTY	,
NULL	as	PAST_VALUATED_STK_QTY	,
NULL	as	PAST_FYR_TM	,
NULL	as	PAST_FSCL_PERD_TM	,
NULL	as	PAST_BLKD_STK_RTRN_QTY	,
NULL	as	PAST_BLKD_STK_QTY	,
NULL	as	PAST_STK_IN_TRNSFR_QTY	,
NULL	as	PAST_PROD_STK_VAL	,
NULL	as	PAST_SLS_VAL
FROM {{source('SAPE03','MARD')}} MARD
  WHERE MARD.MANDT={{var('sape03mandtftr')}}
