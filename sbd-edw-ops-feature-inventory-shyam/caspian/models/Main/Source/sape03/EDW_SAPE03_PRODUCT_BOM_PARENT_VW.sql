...
FROM {{source('SAPE03','MAST')}} MAST
left join {{source('SAPE03','STKO')}} STKO on MAST.STLNR = STKO.STLNR AND MAST.STLAL = STKO.STLAL AND STKO.STLTY = 'M'
WHERE MAST.MANDT={{var('sape03mandtftr')}}