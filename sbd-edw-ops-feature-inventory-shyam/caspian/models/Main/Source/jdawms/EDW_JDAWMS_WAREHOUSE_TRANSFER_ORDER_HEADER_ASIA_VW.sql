select
UPPER(CONCAT(coalesce(invsub.subnum ::VARCHAR,''),'~','JDAWMSASIA')) AS WHSE_TRNSFR_ORD_HDR_KEY,
'JDAWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(invsub.subnum::VARCHAR,''),'~','JDAWMSASIA'))) AS {{var('column_rechashkey')}},
...
FROM {{source('JDAWMSASIA','invsub')}} invsub