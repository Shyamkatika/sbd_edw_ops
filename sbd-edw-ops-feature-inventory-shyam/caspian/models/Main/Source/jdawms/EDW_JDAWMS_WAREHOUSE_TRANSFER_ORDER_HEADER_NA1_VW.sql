select
UPPER(CONCAT(coalesce(invsub.subnum ::VARCHAR,''),'~','JDAWMSNA1')) AS WHSE_TRNSFR_ORD_HDR_KEY,
'JDAWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(invsub.subnum::VARCHAR,''),'~','JDAWMSNA1'))) AS {{var('column_rechashkey')}},
...
FROM {{source('JDAWMSNA1','invsub')}} invsub