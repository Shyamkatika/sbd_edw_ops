SELECT
UPPER(concat(COALESCE(INVDTL.dtlnum::VARCHAR,''),'~','JDAWMSEANZ')) as WHSE_TRNSFR_ORD_LN_KEY,
'JDAWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(concat(COALESCE(INVDTL.dtlnum::VARCHAR,''),'~','JDAWMSEANZ'))) AS {{var('column_redhashkey')}},
...
FROM {{source('JDAWMSEANZ','INVDTL')}} INVDTL