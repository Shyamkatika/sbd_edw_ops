SELECT
UPPER(concat(COALESCE(INVDTL.dtlnum::VARCHAR,''),'~','JDAWMSNA1')) as WHSE_TRNSFR_ORD_LN_KEY,
'JDAWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(concat(COALESCE(INVDTL.dtlnum::VARCHAR,''),'~','JDAWMSNA1'))) AS {{var('column_redhashkey')}},
...
FROM {{source('JDAWMSNA1','INVDTL')}} INVDTL