select 
UPPER(CONCAT(coalesce(CNTHST.CNTHST_ID ::VARCHAR,''),'~','JDAWMSEANZ')) as INVTY_CYCLE_CNT_KEY,
'JDAWMS' AS  {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(CNTHST.CNTHST_ID ::VARCHAR,''),'~','JDAWMSEANZ'))) AS  {{var('column_rechashkey')}},
...
from {{source('JDAWMSEANZ','CNTHST')}} CNTHST 