select
UPPER(CONCAT(coalesce(invsub.subnum ::VARCHAR,''),'~','JDAWMSEANZ')) AS WHSE_TRNSFR_ORD_HDR_KEY,
'JDAWMS' AS {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(invsub.subnum::VARCHAR,''),'~','JDAWMSEANZ'))) AS {{var('column_rechashkey')}},
...
FROM {{source('JDAWMSEANZ','invsub')}} invsub