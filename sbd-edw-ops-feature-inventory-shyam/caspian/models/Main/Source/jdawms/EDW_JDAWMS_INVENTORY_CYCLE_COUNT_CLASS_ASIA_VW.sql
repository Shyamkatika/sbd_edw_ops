select 
UPPER(CONCAT((cnttyp.cnttyp::VARCHAR),'~','JDAWMSASIA')) as CYCLE_CNT_CLS_KEY,
'JDAWMS' AS  {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT((cnttyp.cnttyp::VARCHAR),'~','JDAWMSASIA'))) AS  {{var('column_rechashkey')}},
...
FROM {{source('JDAWMSASIA','CNTTYP')}} CNTTYP