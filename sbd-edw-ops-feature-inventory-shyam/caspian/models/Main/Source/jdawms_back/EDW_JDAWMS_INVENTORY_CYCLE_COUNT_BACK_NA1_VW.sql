select 
UPPER(CONCAT(coalesce(CNTWRK.HASH_FULL_RECORD ::VARCHAR,''),'~','JDAWMSNA1')) as INVTY_CYCLE_CNT_KEY,
'JDAWMS' AS  {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(CNTWRK.HASH_FULL_RECORD ::VARCHAR,''),'~','JDAWMSNA1'))) AS  {{var('column_rechashkey')}},
...
from {{source('JDAWMSNA1','CNTWRK')}} CNTWRK