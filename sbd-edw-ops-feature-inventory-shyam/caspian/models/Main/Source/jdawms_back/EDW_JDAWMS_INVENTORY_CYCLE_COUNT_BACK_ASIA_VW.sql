select 
UPPER(CONCAT(coalesce(CNTWRK.HASH_FULL_RECORD ::VARCHAR,''),'~','JDAWMSASIA')) as INVTY_CYCLE_CNT_KEY,
'JDAWMS' AS  {{var('column_srcsyskey')}},
MD5(UPPER(CONCAT(coalesce(CNTWRK.HASH_FULL_RECORD ::VARCHAR,''),'~','JDAWMSASIA'))) AS  {{var('column_rechashkey')}},
...
from {{source('JDAWMSASIA','CNTWRK')}} CNTWRK