...
-- CASE WHEN UPPER(TRIM(TR_HIST.TR_TYPE)) IN ('CYC-CNT', 'CYC-ERR',' CYC-RCNT', 'ISS-UNP') 
-- OR UPPER(TRIM(TR_HIST.TR_RMKS)) IN ('CYC, RCT-UNP','CYC')
-- THEN  tr_userid ELSE NULL END AS OPER_CD_TYP_LKEY,
...
case when location_id like '9%' or location_id like '7%' then location_id else null end AS WORK_STATION_NBR,
...
DATE(UPISU.CREATED_DATETIME) AS SUMM_ADJ_RESN_CD_DTE,
in_mstr.in_wh AS WMS_SCHEMA_NAME
--count(location_id) AS SLOC_CNT,
...
-- LEFT JOIN ( SELECT DISTINCT count(location_id) AS SLOC_CNT, FROM {{source("EXCEL_ERRORPROOF","UPISU")}}) UPISU_TEMP
WHERE UPPER(TRIM(IN_DOMAIN))= 'EXCEL'
and UPISU.quantity <>0 AND UPPER(TRIM(UPISU.remarks)) = 'INV COUNT'