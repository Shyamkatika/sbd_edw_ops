...
CASE WHEN UPPER(PTPDET.ptp_ins_rqd)='YES' then 'Y' 	ELSE 'N' end as	INCM_INSPCT_FLAG	,
CASE WHEN UPPER(PTPDET.ptp_iss_pol)='YES' then 'Y' 	ELSE 'N' end as	PICK_LIST_FLAG,

PTPDET.ptp_mfg_lead	as	PROD_MFG_DY_CNT	,
CASE WHEN UPPER(PTPDET.ptp_ms)='YES' then 'Y' 	ELSE 'N' end as	PROD_APPEARED_IN_SCHED_FLAG,
...
FROM {{source('QADAR','PTPDET')}} PTPDET