...
FROM {{source('SAPP10','MAST')}} MAST
left join {{source('SAPP10','STKO')}} STKO on MAST.STLNR = STKO.STLNR AND MAST.STLAL = STKO.STLAL AND STKO.STLTY = 'M'
WHERE MAST.MANDT={{var('sapp10mandtftr')}}