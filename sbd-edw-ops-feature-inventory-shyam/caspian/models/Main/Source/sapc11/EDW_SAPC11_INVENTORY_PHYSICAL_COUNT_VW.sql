...
IKPF.XBLNI AS PHY_INVTY_REF_NBR,
 CASE WHEN IKPF.ZSTAT = 'X' THEN 'Y' ELSE 'N' END AS CNT_STAT_FLAG,
...
--ISEG.ERFME AS CNT_ENTR_UOM_KEY,
...
FROM {{source('SAPC11','IKPF')}}  IKPF 
LEFT JOIN {{source('SAPC11', 'ISEG')}} ISEG ON IKPF.IBLNR = ISEG.IBLNR AND IKPF.GJAHR = ISEG.GJAHR
WHERE IKPF.MANDT={{var('sapc11mandtftr')}}