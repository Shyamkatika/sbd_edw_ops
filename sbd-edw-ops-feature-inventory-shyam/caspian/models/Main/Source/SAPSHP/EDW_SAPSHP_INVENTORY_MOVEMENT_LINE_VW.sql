...
MSEG.MATBF AS STK_MANAGED_PROD_NBR,
...
case when length(trim(UPPER((MSEG.ZZ_LAND1)))) <1 then {{var('default_mapkey')}} when UPPER(MSEG.ZZ_LAND1)
IS NULL then {{var('default_mapkey')}} else UPPER(MSEG.ZZ_LAND1) end AS ENTY_CNTRY_KEY,
...
TRY_TO_TIMESTAMP_NTZ(CONCAT(TRY_TO_DATE(MSEG.CPUDT_MKPF,'YYYY.MM.DD'),' ',
            TRY_TO_TIME((case when length(trim(MSEG.CPUTM_MKPF)) <1 then '00:00:00'
when MSEG.CPUTM_MKPF IS NULL then '00:00:00'
else MSEG.CPUTM_MKPF end),'HH24MISS'))) as ENTR_TM,

--TRY_TO_TIMESTAMP_NTZ(CONCAT(TRY_TO_DATE(MSEG._BEV2_ED_AEDAT,'YYYYMMDD'),' ',
--           TRY_TO_TIME((case when length(trim(MSEG._BEV2_ED_AETIM)) <1 then '00:00:00'
--when MSEG._BEV2_ED_AETIM IS NULL then '00:00:00'
--else MSEG._BEV2_ED_AETIM end),'HH24MISS'))) as EXCISE_DUTY_CURR_SYS_TM,



--TRY_TO_DATE(MSEG._BEV2_ED_AEDAT,'YYYY.MM.DD') AS EXCISE_DUTY_LAST_UPD_DTE,
TRY_TO_DATE(MSEG.BUDAT_MKPF,'YYYY.MM.DD') AS FIN_ACCT_DTE,
TRY_TO_DATE(MSEG.DABRBZ,'YYYY.MM.DD') AS SETLMNT_REF_1_DTE,
TRY_TO_DATE(MSEG.DABRZ,'YYYY.MM.DD') AS SETLMNT_REF_2_DTE,
TRY_TO_DATE(MSEG.HSDAT,'YYYY.MM.DD') AS PROD_MFG_DTE,
TRY_TO_DATE(MSEG.VFDAT,'YYYY.MM.DD') AS SHELF_LIFE_EXPR_DTE,
TRY_TO_DATE(MSEG.CPUDT_MKPF,'YYYY.MM.DD') AS ACCT_DOC_DTE_ENTR_TM,
TRY_TO_DATE(MSEG.FBUDA,'YYYY.MM.DD') AS SVCS_RENDERED_DTE,
...
MSEG.ZZ_REP_ENT AS RPT_ENTR_NBR,
MSEG.ZZ_KUNNR_S AS SENDER_CUST_NAME,
MSEG.ZZ_BELNR_S AS SENDER_DOC_NBR,
MSEG.ZZ_LIFNR_S AS SENDER_VEND_NAME,
MSEG.ZZ_PAYR_S AS SENDER_PAYER,
MSEG.DUMMY_INCL_EEW_COBL AS DUMMY_FIELD,
MSEG.SGT_SCAT AS STOCKABLE_SEG_PROD_NBR,
MSEG.SGT_UMSCAT AS RCV_STK_SEG,
MSEG.SGT_RCAT AS REQ_SEG,
MSEG.SERVICEPERFORMER AS SVC_PERFORMER,
MSEG.PERNR AS PRSN_NBR,
MSEG.KNTTP_GR AS ACCT_ASGN_CTGY,
MSEG.WORK_ITEM_ID AS WORK_ITEM_NBR,
MSEG.XPRINT AS PRINT_VIA_OUTPUT_CTRL,
MSEG.SERVICE_DOC_TYPE AS SVC_DOC_TYP,
MSEG.SERVICE_DOC_ID AS SVC_DOC_NBR,
MSEG.SERVICE_DOC_ITEM_ID AS SVC_DOC_ITEM_NBR,
MSEG.EWM_LGNUM AS WHSE_NBR,
MSEG.EWM_GMDOC AS GOODS_MVMNT_DOC,
MSEG.DISUB_OWNER AS STK_OWN_NAME,

MSEG.FSH_SEASON_YEAR AS ARTICLE_SOLD_SEASON_YR,
MSEG.FSH_SEASON AS FASHION_ARTICLE_SEASON,
MSEG.FSH_COLLECTION AS FASHION_ARTICLE_CLCTN,
MSEG.FSH_THEME AS FASHION_THEAM,
MSEG.FSH_UMSEA_YR AS ISS_SEASON_YR,
MSEG.FSH_UMSEA AS ISS_SEASON_NAME,
MSEG.FSH_UMCOLL AS ISS_CLCTN,
MSEG.FSH_UMTHEME AS ISS_THEME,
MSEG.SGT_CHINT AS DISCRETE_BATCH_NBR,
MSEG.COMPL_MARK AS SHIP_TYP,
MSEG.FZGLS_MARK AS CUML_RCV_QTY,
MSEG.ETANP_MARK AS SCHED_AGRMT_ITEM_QTY,
MSEG.POPUP_MARK AS DISPLAY_DIALOG_BOX,
MSEG.WRF_CHARSTC1 AS ARTICLE_CHAR_1_VAL,
MSEG.WRF_CHARSTC2 AS ARTICLE_CHAR_2_VAL,
MSEG.WRF_CHARSTC3 AS ARTICLE_CHAR_3_VAL,
MSEG._CWM_MENGE AS PARALLEL_UOM_QTY,
MSEG._CWM_MEINS AS PARALLEL_UOM,
MSEG._CWM_ERFMG AS PARALLEL_UNIT_ENTR,
MSEG._CWM_ERFME AS PUOM_ENTR_UOM,
...
FROM {{source('SAPSHP','MSEG')}} MSEG
WHERE MSEG.MANDT={{var('sapshpmandtftr')}}