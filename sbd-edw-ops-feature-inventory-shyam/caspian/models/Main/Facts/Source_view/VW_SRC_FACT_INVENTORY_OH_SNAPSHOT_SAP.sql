...
WITH DIM_PRODUCT_LOC_VALUATION_DELTA AS 
(SELECT PROD_KEY,VALTN_AREA_LKEY,VALTN_TYP_LKEY,FYR_NBR,FMTH_NBR,STANDARD_PRICE,
UNIT_PRICE,VALTN_CLASS,CO_KEY,CRNCY_KEY,SRC_SYS_KEY FROM (SELECT PROD_KEY,VALTN_AREA_LKEY,VALTN_TYP_LKEY,FYR_NBR,FMTH_NBR,STANDARD_PRICE,UNIT_PRICE,VALTN_CLASS,CO_KEY,CRNCY_KEY,SRC_SYS_KEY FROM(
...
WHERE row_number=1) WHERE SRC_SYS_KEY IN ('SAPC11','SAPE03'))
...
,OH.REPL_QTY
,(OH.REPL_QTY * (CASE WHEN OH.STANDARD_PRICE<>0 AND OH.UNIT_PRICE <>0 THEN (OH.STANDARD_PRICE/OH.UNIT_PRICE) 
ELSE 0 END)) AS REPL_AMT
...
,OH.PAST_FSCL_PERD_TM
,OH.PROD_ADD_LOC_YR_ID
,OH.PROD_REL_SET_ITEM,PLV.VALTN_AREA_LKEY,PLV.VALTN_TYP_LKEY,PLV.FYR_NBR,PLV.FMTH_NBR,PLV.STANDARD_PRICE,PLV.UNIT_PRICE,PLV.VALTN_CLASS,PLV.CO_KEY,PLV.CRNCY_KEY FROM
(SELECT * FROM {{source('CONSOLIDATED','EDW_INVENTORY_ON_HAND_SNAPSHOT')}} 
WHERE  ETL_INS_PID  NOT LIKE '%HIST%' AND SRC_SYS_KEY IN ('SAPC11','SAPE03') AND CONCAT(SRC_SYS_KEY,'~',INVTY_ON_HAND_SNAPSHOT_KEY) NOT IN
(SELECT CONCAT(SRC_SYS_KEY,'~',INVTY_ON_HAND_SNAPSHOT_KEY) FROM {{source('FACTS','FACT_INVENTORY_ON_HAND_SNAPSHOT')}} WHERE 
ZONE4_ETL_INS_PID  LIKE '%HIST%' AND SRC_SYS_KEY IN ('SAPC11','SAPE03'))) OH 
LEFT JOIN DIM_PRODUCT_LOC_VALUATION_DELTA PLV ON 
UPPER(concat(OH.PROD_KEY,'~',OH.LOC_KEY,'~',OH.CURR_FYR,'~',OH.CURR_CLNDR_PERD_NBR)) = 
...
,OH1.PROD_REL_SET_ITEM,PLV1.VALTN_AREA_LKEY,PLV1.VALTN_TYP_LKEY,PLV1.FYR_NBR,PLV1.FMTH_NBR,PLV1.STANDARD_PRICE,PLV1.UNIT_PRICE,PLV1.VALTN_CLASS,PLV1.CO_KEY,PLV1.CRNCY_KEY FROM 
(SELECT OH.* FROM 
(SELECT * FROM {{source('CONSOLIDATED','EDW_INVENTORY_ON_HAND_SNAPSHOT')}}  
WHERE  ETL_INS_PID  NOT LIKE '%HIST%'AND SRC_SYS_KEY IN ('SAPC11','SAPE03') AND  CONCAT(SRC_SYS_KEY,'~',INVTY_ON_HAND_SNAPSHOT_KEY) NOT IN
(SELECT CONCAT(SRC_SYS_KEY,'~',INVTY_ON_HAND_SNAPSHOT_KEY) FROM {{source('FACTS','FACT_INVENTORY_ON_HAND_SNAPSHOT')}} WHERE 
ZONE4_ETL_INS_PID  LIKE '%HIST%' AND SRC_SYS_KEY IN ('SAPC11','SAPE03'))) OH LEFT JOIN DIM_PRODUCT_LOC_VALUATION_DELTA PLV ON 
UPPER(concat(OH.PROD_KEY,'~',OH.LOC_KEY,'~',OH.CURR_FYR,'~',OH.CURR_CLNDR_PERD_NBR)) = 
UPPER(concat(PLV.PROD_KEY,'~',PLV.VALTN_AREA_LKEY,'~',FYR_NBR,'~',PLV.FMTH_NBR)) AND OH.SRC_SYS_KEY = PLV.SRC_SYS_KEY 
WHERE PLV.PROD_KEY iS  NULL AND PLV.VALTN_AREA_LKEY IS  NULL) OH1 LEFT JOIN DIM_PRODUCT_LOC_VALUATION_DELTA PLV1 ON 
UPPER(concat(OH1.PROD_KEY,'~',OH1.LOC_KEY))= UPPER(concat(PLV1.PROD_KEY,'~',PLV1.VALTN_AREA_LKEY)) AND 
CONCAT(PLV1.FYR_NBR,PLV1.FMTH_NBR)< CONCAT(OH1.CURR_FYR,OH1.CURR_CLNDR_PERD_NBR)  AND OH1.SRC_SYS_KEY = PLV1.SRC_SYS_KEY 
